...
from fasthep_curator.config import (
    CuratorConfig,
    Dataset,
    compact,
    load_config,
...
def test_compact():
    cfg = CuratorConfig(
        datasets=[
            Dataset(name="foo", eventtype="mc", tree="events"),
            Dataset(name="bar", eventtype="mc", tree="events"),
        ],
        metadata={"a": 1},
    )
    ...
    assert cfg.defaults == {"eventtype": "mc", "tree": "events", "metadata": {}}
    assert len(cfg.datasets) == 2
    assert all(d.eventtype is None for d in cfg.datasets)
    assert all(d.tree is None for d in cfg.datasets)
